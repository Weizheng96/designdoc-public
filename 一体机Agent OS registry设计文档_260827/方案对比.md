...
无论哪套方案，注册中心镜像注册表的**一行（= 一个 `name` 的一个 `version`）**都由**三类元数据**构成——这也是理解两方案分歧的基础：**分歧只落在第①类（元戎所需）由谁持有**，第②③类两方案完全相同、始终留在注册中心。
...
| ② | **版本管理元数据** | 多版本与默认版本管理、检索与排序 | `name`（主键）· `framework`（展示）· `version` · `version_key` · `is_default` · `image_module_version` · `access_mode` |
| ③ | **用户信息元数据** | 归属与审计 | `uploaded_by`（上传者）· `created_at`（登记时间）· `description` · `package_path` · `image_archive_path` |
...
1. **元戎函数名 `name`（如 `0@myService@python-agent`）由谁生成？**
   - 选项 A：注册中心按 `name` + `version` 确定性派生（与 `service_id` 风格一致，镜像处理模块无需关心）。
