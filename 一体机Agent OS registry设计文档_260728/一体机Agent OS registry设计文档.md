...
>
> **存储恒为 rqlite**——单机是单节点 rqlite（自身即 leader），多机是多节点 rqlite（Raft 复制 + 选主）；不存在本地 SQLite 分支，单机扩多机不迁数据、不改代码路径。**注册中心主备跟随 rqlite 的 Raft 选主**：与 rqlite leader 同机的注册中心即为主，备只待命；**心跳租约只存在于主的内存中，不入库**，故必须单活。单机 gateway 直连注册中心；多机由**入口节点的一台 nginx** 转发，新主主动把 nginx 的 upstream 指向自己。
>
...
| | 租约策略 | 运维 | 读 / 改节点心跳的开关、TTL 与宽限期（`lease-config`）|
| **平台** | 独立自启 | systemd | 有 `registry.env` 即起注册中心；无则该节点为纯 gateway 节点 |
| | 存储 | 注册中心 | **恒为 rqlite**：单机单节点、多机多节点，同一套 SQL 与建表约束 |
| | **主备** | 注册中心 | 跟随 rqlite 的 Raft 选主：与 leader 同机者为主，备停扫描、健康检查返 503 |
| | **统一入口** | nginx | 多机形态下入口节点一台 nginx，新主主动把 upstream 指向自己（单机无 nginx）|
| | 鉴权 | 注册中心 | 静态 API Key；镜像 / 实例 / 心跳接口受 token 门控 |
...
- **注册中心单活**——心跳租约在主的进程内存中，备实例若同时运行会因租约表为空而误删实例；故任一时刻只有一个注册中心对外服务（见 [§3.3.3](#333-多机形态主备--nginx)）。
...
| **心跳管理** | `heartbeat/` | 节点级心跳续租 / 状态派生 / 超宽限批量剔除；租约表在内存 |
| **鉴权** | `auth/` | 静态 API Key 签发与校验 |
| **注册表** | rqlite | 镜像注册表 `images` / 实例注册表 `instances`（单机单节点、多机多节点）|

**外部模块**：网页前端 / 用户 · 九问 gateway · **元戎**（Docker 沙箱；**由 gateway 调用拉起 / 停止**，注册中心不调用）· 三方 / 九问 agent 实例 · 一体机镜像仓 · **镜像处理模块**（把原始镜像适配为可运行镜像**并登记进注册中心**；由用户前端触发）· **rqlite**（存储，与注册中心同机）· **nginx**（多机形态的统一入口）。
...
> 每个场景标注**外部交互接口**（方法 + 路径、输入示例、输出示例）；完整请求 / 响应 / 错误契约见 [`registry_openapi.yaml`](./registry_openapi.yaml)。镜像 / 实例为**固定命名注册表**接口（`/api/images`、`/api/instances`，不分 dataset——数据落固定的 `images` / `instances` 注册表）；node 心跳、租约策略、ha 为全局。
...
Agent 实例（三方 / 九问）的注册 / 变更 / 注销 / 查询 / 存活监控。**gateway 自行决定是否拉起**（不查注册中心）：拉起前经 [§3.1.3](#313-取运行规格) 取运行规格 → 自行调元戎拉起 → 向注册中心**注册**；落点变化时**变更**；关闭时自行停元戎后**注销**。**用户**查询实例及状态。三方与九问同一套流程，仅九问框架镜像条目预置。**注册中心不调元戎**，是纯记录方。
...
- **输入示例**：`{ "node": "192.168.0.20", "address": "10.244.3.9:4096" }`（`node` / `address` 至少给一个，否则 `400`）
- **输出示例**：`{ "service_id": "generic_3f9a1b2c", "kind": "三方", "framework": "opencode", "framework_version": "v0.2.0", "address": "10.244.3.9:4096", "node": "192.168.0.20", "user": "user-01", "status": "运行" }`
...
用户经 UI 查实例及**状态**。查状态须带 `?include_unhealthy=true`（默认只回 `运行`，该参数才把失活派生为 `异常`）；可按 `node` / `framework` / `kind` / `user` 过滤（`?user=<ID>` 查某用户的实例）。支持**分页**（`size` / `page`，同 [§3](#3-场景逻辑视图) 分页约定；**一页 = N 个实例**）。
...
- **过滤与分页的先后**：先按过滤条件（含 `include_unhealthy` 决定的状态可见性）筛出全集，再对结果分页，故 `X-Total-Count` 是**过滤后**的总数。`include_unhealthy=false` 的过滤下推成 SQL 的 `node NOT IN (失活 node 集)`，因此计数与翻页都在库里完成
...
> **端到端尚未打通**：服务端接口与「默认关闭」已实现，gateway 侧心跳发送与客户端 SDK 封装仍在开发中，故 B020 不交付本能力；具体状态见 [`开发进展.md`](./开发进展.md)。

```mermaid
flowchart TB
    GW([九问 gateway]):::ext
    subgraph REG["九问 Agent 注册中心 · 主"]
        direction TB
        BE["后端分发"]:::mod
        HB["心跳管理<br/>租约表在内存"]:::mod
        AM["实例管理"]:::mod
...
**接口 ① 节点心跳**：`POST /api/nodes/{node}/heartbeat`（发起方：gateway；`node` = 本机 nodeIP，节点级：一次覆盖该 node 全部实例）

- **输入示例**：无 body（可选 `{ "status": null }`）
- **输出示例**：`{ "node": "192.168.0.12", "state": "healthy", "ttl_seconds": 90, "expires_at": 1751800000.0 }`

**接口 ② 租约策略**：`GET /api/lease-config` / `POST /api/lease-config`（发起方：运维；全局，非按节点）

- **输出示例**：`{ "enabled": false, "min_ttl": 90, "max_ttl": 3600, "grace_period": 30 }`
- `enabled` 是**存活监控的总开关，默认关闭**——关闭时不收心跳、不派生 `异常`、不做超宽限剔除，实例条目只由 gateway 显式注销来删除；`min_ttl` 兼作节点租约的实际 TTL（gateway 不带 TTL 发心跳）；`grace_period` 是超 TTL 后的可恢复宽限期

**状态机**（`enabled=true` 时，每个 node 一份租约）：

| 迁移 | 触发 | 效果 |
|------|------|------|
| 无租约 | 实例已注册但 gateway 尚未首发心跳 | 视为健康，`status = 运行`（不误删新注册的实例）|
| `healthy` → `unhealthy` | TTL（默认 90s）内未续 | 其上实例 `status = 异常`；默认查询不返回 |
| `unhealthy` → `healthy` | 宽限期（默认 30s）内收到心跳 | 软恢复，`status` 回 `运行` |
| `unhealthy` → 删除 | 超宽限 | 心跳管理调实例管理删该 node **全部**实例条目 |

> **租约不入库**：租约表在注册中心进程内存，不写 rqlite。因此**注册中心重启或主备切换后，所有有实例的 node 会被置为 `unhealthy` 并开始一个宽限窗**——gateway 在窗口内重发心跳即全部恢复；窗口内实例显示为 `异常` 但不被删。这是「注册中心必须单活」的直接原因（见 [§3.3.3](#333-多机形态主备--nginx)）。也正因为这条路径会真删数据，**存活监控默认关闭**，须在 gateway 侧心跳接入后再由运维显式打开。
...
| 角色 | 判定 | 行为 |
|------|------|------|
| **主** | 本机 rqlite 是 Raft leader | 正常服务；**心跳扫描运行**；把入口 nginx 的 upstream 指向自己 |
| **备** | 本机 rqlite 是 follower | **停心跳扫描**；健康检查返 `503`，不接业务流量 |

> **备为什么必须停心跳扫描**：租约表在内存、不入库（[§3.2.5](#325-存活监控)）。备的租约表是空的，若它也在扫，就会认定所有 node 超宽限，进而**把主刚写进 rqlite 的实例条目删掉**——两者共用同一份 rqlite，删除是真删。所以「单活」不是性能取舍，是数据安全要求。
>
> **切主的观察窗**：新主接管时租约表为空，所有有实例的 node 会进入 `unhealthy` + 一个 `grace_period`（默认 30s）的宽限窗，期间实例显示 `异常` 但不删；gateway 在窗口内重发心跳即全部恢复。这条路径与「注册中心重启」完全相同。
...
flowchart TB
    ...
    subgraph MB["注册中心节点 B · 备"]
        RB["注册中心 :4003<br/>停扫描 · 健康检查 503"]:::off
        QB[("rqlite · follower")]:::reg
        RB --- QB
    end
    subgraph MC["注册中心节点 C · 备"]
        RC["注册中心 :4003<br/>停扫描 · 健康检查 503"]:::off
...
> **rqlite 的读写不需要注册中心感知主**：rqlite 的 follower 收到写请求会经节点间通道转发给 leader，读同理。所以注册中心恒连**本机** `127.0.0.1:4001` 即可，选主对存储访问是透明的。注册中心之所以还要知道谁是主，**只是为了决定自己是否该对外服务**（租约在内存，见上）。
...
3. **注册中心 4003 启用鉴权** —— 静态 API Key；镜像 / 实例 / 心跳接口均受 token 门控，gateway 与镜像处理模块各持自己的 token。多机形态下即使 4003 绑在业务网卡上（nginx 需跨机转发到主，故不能绑 `127.0.0.1`），也不会裸奔。
...
> 不允许 `0.0.0.0`：单机绑业务网卡供 gateway 直连；多机同样绑业务网卡（nginx 需跨机转发到当前主，绑回环则打不到）。安全由 [§3.3.4](#334-端口与安全) 的鉴权 + 防火墙保证。
...
**注册表 `instances`**（kind=instance，物理表 `instance`）：一行 = 一个实例，`service_id` 由 **(user, framework) 确定性派生**（每用户每框架**一个实例**）。`status` 不落库、查询时据 node 心跳派生。
...
|---|---|---|---|---|---|---|---|
| instances | `generic_3f9a…` | 三方 | opencode | v0.2.0 | 192.168.0.12 | user-01 | `{"address":"10.244.1.7:4096","created_at":"2026-07-06T10:00:00Z","last_active_at":"2026-07-06T10:42:00Z"}` |
| instances | `generic_9c21…` | 九问 | jiuwen-report | v1.0.0 | 192.168.0.11 | user-02 | `{"address":"10.244.2.3:8080","created_at":"…","last_active_at":"…"}` |

> **gateway 注册 / 变更 / 注销**：自行经元戎拉起 / 停止，带落点（`node` / `address`）写入；变更改 `node` / `address`。`node` = 实例实际所在节点 IP（元戎落点 nodeIP，可能非发起方所在一体机）。**只存活条目**：`status` 查询时据 node 心跳派生——`运行`（TTL 内 / 无租约 / 存活监控关闭）/ `异常`（超 TTL、宽限内可恢复）；**超宽限**或 **gateway 注销**则删条目（不留终态）。**查询顺序**：`framework ASC, "user" ASC, service_id ASC`（`service_id` 为主键，兜底保证全序——分页所需）；`status` 派生自心跳、不落库，故**不参与排序**。
...
| **实现文档** | [`一体机Agent OS registry实现文档.md`](./一体机Agent%20OS%20registry实现文档.md) | ① 整体项目架构；② 各模块功能与接口（依照开发顺序）。**注意**：该文档尚未随本次设计更新（仍为上一版），以本文档与 OpenAPI 契约为准 |
