...
- 本方案聚焦 **Gateway + 注册中心** 两个组件的高可用。
- 附件（上传图片）的跨节点同步作为**遗留问题**（见 §4.4），本方案不阻塞主流程落地，但数据同步场景必须完整记录。
- 注册中心数据库后端：**单机临时用 sqlite，后续改 etcd**（组网已有公共 etcd 可复用）。etcd 本身是分布式存储，天然提供数据多副本与高可用；sqlite 仅是过渡形态。
